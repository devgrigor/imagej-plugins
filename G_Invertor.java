...
A few things to note:
	...
	5) The class name ("GInvertor") and file name 
	("GInvertor.java") must be the same.
...
public class G_Invertor implements PlugInFilter {
	ij.ImagePlus imp;
	...
	public void run(ImageProcessor ip) {
		...
		for (int y=r.y; y<(r.y+r.height); y++) {
			...
			for (int x=r.x; x<(r.x+r.width); x++) {
				if(x < r.height && y < r.width) {
					ip.set(x, y, ~ip.get(y,x));	
				}
